...
class DataCheckWorker(
    ...
    override suspend fun doWork(): Result {
        return try {
            ...
            if (dayOfMonth in 14..15 && !NotificationTracker.hasBeenSent(applicationContext, NotificationTracker.TYPE_MID_MONTH)) {
                ...
                if (currentUsage > expectedUsageAtMidMonth * 1.3) { // 30% over pace
                    ...
                    NotificationTracker.markAsSent(applicationContext, NotificationTracker.TYPE_MID_MONTH)
                }
            }
            
            // Check 2: 90% of monthly budget used
            val percentUsed = (currentUsage / monthlyBudget) * 100
            if (percentUsed >= 90 && percentUsed < 100 && !NotificationTracker.hasBeenSent(applicationContext, NotificationTracker.TYPE_90_PERCENT)) {
                NotificationHelper.sendNotification(
                    applicationContext,
                ...
                NotificationTracker.markAsSent(applicationContext, NotificationTracker.TYPE_90_PERCENT)
            }
            
            // Check 3: Running low on TOTAL remaining data (< 20% left)
            val percentOfTotalRemaining = (actualRemaining / config.totalDataGB) * 100
            if (percentOfTotalRemaining < 20 && percentOfTotalRemaining > 10 && !NotificationTracker.hasBeenSent(applicationContext, NotificationTracker.TYPE_LOW_DATA)) {
                NotificationHelper.sendNotification(
                    applicationContext,
                ...
                NotificationTracker.markAsSent(applicationContext, NotificationTracker.TYPE_LOW_DATA)
            }
            
            // Check 4: End of month good job (last day of month, under budget)
            if (dayOfMonth == daysInMonth && currentUsage < monthlyBudget && !NotificationTracker.hasBeenSent(applicationContext, NotificationTracker.TYPE_MONTH_END)) {
                ...
                NotificationTracker.markAsSent(applicationContext, NotificationTracker.TYPE_MONTH_END)
            }
            
            // Check 5: Over 100% of monthly budget
            if (percentUsed >= 100 && !NotificationTracker.hasBeenSent(applicationContext, NotificationTracker.TYPE_OVER_BUDGET)) {
                ...
                NotificationTracker.markAsSent(applicationContext, NotificationTracker.TYPE_OVER_BUDGET)
            }
